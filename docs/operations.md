...
2. 마이그레이션은 `vercel.json`의 `buildCommand`(`prisma migrate deploy && next build`)가
   매 배포마다 자동 실행한다 — `DATABASE_URL`이 빌드 시점에 유효하기만 하면 된다. 초기
   마이그레이션(`prisma/migrations/0_init`)은 저장소에 포함돼 있어 빈 DB에 바로 적용된다.
   (Vercel 외 환경에 배포한다면 `npx prisma migrate deploy`를 직접 실행.)
