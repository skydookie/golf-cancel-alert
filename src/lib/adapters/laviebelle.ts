...
export const laviebelleOldCourseAdapter: SiteAdapter = {
  ...
  buildDeepLink(date): string {
    // T9 "확인 필요 사항 2"의 조사 결과에 따라, 날짜별 직접 링크가 가능하면 쿼리 파라미터 방식으로
    // 연결을 시도한다. 실사이트가 이 파라미터를 지원하지 않으면(폼 제출/세션 기반 네비게이션만
    // 가능하면) 캘린더 메인 화면으로 폴백해야 한다 — 이 부분은 T9에서 실사이트 확인 후 이 함수를
    // 조정한다(현재는 캘린더 메인 화면으로의 폴백을 기본값으로 둔다. 아래 참고).
